...
def shortest_path(source, target):
    ...
    #if ids are the same, return None
    if target == source:
        return None
    ...
    while not connection_:
        ...
        # ->node.state is target, return the connection_  to it
        if node.state == target:
            connection_ = True
            while node.parent is not None:
                so, ta = node.state, node.action
                connection.append((ta, so))
                node = node.parent
            connection.reverse()


        # add node state to states done
        ...
        for neighbor in neighbors:
            ...
            if child.state == target:
                ...
                while child.parent is not None:
                    so, ta = child.state, child.action
                    connection.append((ta, so))
                    child = child.parent
                connection.reverse()
                return connection
        ...
        for action, state in neighbors:
            if not frontier_queue.contains_state(state) and state not in nodes_done:
    ...
    """
    Complete the implementation of the shortest_path function such that it returns the shortest path from 
    the person with id source to the person with the id target. 

    • Assuming there is a path from the source to the target, your function should return a list, where 
    each list item is the next (movie_id, person_id) pair in the path from the source to the target. 
    Each pair should be a tuple of two ints. 

    For example, if the return values of shortest_path were [(1, 2), (3, 4)], that would mean that 
    the source starred in movie 1 with person 2, person 2 starred in movie 3 with person 4, and 
    person 4 is the target. 

    • If there are multiple paths of minimum length from the source to the target, your function can 
    return any of them. 
